...
namespace FinMan.Data
{
    public class CustomerClient
    {
        ...
        public async Task<UpdateCustomerRequestResponse> UpdateCustomer(string customerId, string phoneNumber, string email, /*string street, string postalCode, string city, string country*/ Address address)
        {
            UpdateCustomerRequestResponse customer = new UpdateCustomerRequestResponse { customerID = customerId, email = email, phoneNumber = phoneNumber, address = address/*new Address { street = street, postalCode = postalCode, city = city, country = country } */};
            var client = await Utils.GetClient();
            var result = await client.PostAsync(Url, new StringContent(JsonConvert.SerializeObject(customer), Encoding.UTF8, "application/json"));
